...
int Search(BTree T,int key)//返回要查找值的前驱父节点
{
    BTree p=T;
    int endnum;
    if(p)						//树不为空时
...
void split(BTree &q,int s,BTree &ap)//分裂节点
...
int InsertBTree(BTree &T,int K,BTree q,int i){
    ...
    while(q&&!finished){
        ...
        if (q->keynum<m)
            ...
            //将q->key[s+1..m], q->ptr[s..m]移入新结点*ap
            q=q->parent;
            if(q)
            {
...
Result SearchBTree(BTree &T, int key){
    /*在m阶B-树T上查找关键字key，返回结果(pt,i,tag)。若查找成功，则特征值tag=1，
     指针pt所指结点中第i个关键字等于key；否则特征值tag=0，
     等于key的关键字应插入在指针pt所指结点中第i和第i+1个关键字之间*/
    BTree p=T;
    BTree q=NULL;
    int found=FALSE;
    int i=0;			//初始化，p指向待查结点，q指向p的双亲
    while(p&&!found){
        i=Search(p,key);//查找该节点是否有待查值，返回元素位置，或者下一节点的指针下标
